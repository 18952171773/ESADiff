...
#from denoising_diffusion_pytorch.S3_DSConv_pro import DSConv_pro
from denoising_diffusion_pytorch.DCN import DCNConv
from denoising_diffusion_pytorch.DCNv2 import DeformConv2d
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups = 8):
        ...
        #     True,
        # )
        self.conv1 = EncoderConv(2 * dim_out, dim_out)
    ...
    def forward(self, x, scale_shift = None):
        ...
        #xx=self.conv0x(x)
        #xy = self.conv0y(x)
        #x = self.conv1(torch.cat([x0, xx], dim=1))
       # x = self.maxpooling(x01)
        # 打印每个张量的尺寸
        #print(f"x0 shape: {x0.shape}, x shape: {x.shape}, x01 shape: {x01.shape}")
        x = self.norm(x)
        ...
        x = self.act(x)
        xx=self.conv0x(x0)
        #print(f"x shape: {x.shape}, xx shape: {xx.shape}")
        #xy = self.conv0y(x)
        x1 = self.conv1(torch.cat([x, xx], dim=1))
        
        # 打印每个张量的尺寸
        #print(f"x shape: {x.shape}, xx shape: {xx.shape}, x1 shape: {x1.shape}, x2 shape: {x2.shape}")
