...
def main(args):
    ...
    # experiment = wandb.init(project='Diffusion', resume='allow', entity='1143284345')
    ...
    def train(self):#,experiment):
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                total_loss = 0.
                total_loss_dict = {'loss_simple': 0., 'loss_vlb': 0., 'loss_eprl': 0.,'total_loss': 0., 'lr': 5e-5}
                for ga_ind in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        ...
                        loss_eprl = log_dict["train/loss_eprl"].item() / self.gradient_accumulate_every
                        total_loss_dict['loss_eprl'] += loss_eprl
                ...
                currentloss=total_loss_dict['total_loss']
                if accelerator.is_main_process:
                ...
                #     'loss_eprl': total_loss_dict['loss_eprl'],  # 使用累加后的值
                # })
                accelerator.wait_for_everyone()
                ...
                if accelerator.is_main_process:
                    self.ema.to(device)
                    self.ema.update()
                    if self.step != 0 and self.best_loss > currentloss and self.step//self.save_and_sample_every>=28:
                        self.best_loss=currentloss
                        milestone = 0
                        self.save(milestone)
                        self.model.eval()
